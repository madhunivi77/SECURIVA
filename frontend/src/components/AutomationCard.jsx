...
const AutomationCard = ({ 
  ...
  return (
    <div 
      className="p-5 border rounded-2xl border-gray-300 cursor-pointer"
    >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-300">
        ...
        <div className="flex items-center gap-2">
            {/* # of Runs */}
          <span className="text-sm">{triggerCount} runs</span>

          {/* Pause Button */}
          <button className="flex items-center justify-center h-8 w-8 text-gray-500 hover:text-black hover:bg-blue-200">
            {isActive ? <Pause className="h-4 w-4 shrink-0" /> : <Play className="h-4 w-4 shrink-0" />}
          </button>
        </div>
      </div>
    </div>
